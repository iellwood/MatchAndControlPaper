...
# thresholds = {
#     0.5:    6.039219977285495e-14,
#     1:      7.868762741685457e-14,
#     2:      1.0341419360815979e-13,
# }
...
def plot_hist(i):
    ...
    a = np.reshape(data_random_spines, [-1])/thresholds[i]
    b = np.reshape(data_matched_spines, [-1])/thresholds[i]
...
def make_roc(negatives, positives, color='k', label=None, include_45_line=True):
    ...
    fpr, tpr, thresh = sklearn.metrics.roc_curve(y_true, y_score, sample_weight=sample_weight, pos_label=1)

    #i = np.argmin(np.square(fpr - 0.001486517711815405))
    i = np.argmin(np.square(fpr - 0.1))


    print('for fpr =', fpr[i], 'true positive rate =', tpr[i], 'threshold =', thresh[i])
...
for i in range(len(paths)):
    ...
    thresholds[i] = make_roc(data_random_spines_max, np.reshape(data_matched_spines, [-1]), color=prettyplot.color_list[i], label=[0.5, 1, 2][i], include_45_line=(i==0))
...
np.savez('../SimulationData/thresholds.npz', thresholds=thresholds)
